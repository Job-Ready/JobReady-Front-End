...
function Design() {
  ...
  const getResumeIndex = resumes.reduce((acc, curr, index) => {
    ...
    return index === 0 ? index : (curr.last_change > resumes[acc].last_change ? index : acc);
  }, 0);
            ...
            <div className='flex justify-between'>
              ...
              <div className="mb-4 items-center">
                ...
                <div className="flex border rounded-lg w-max">
                  <button onClick={() => setFontSize(prevSize => `${Math.max(parseInt(prevSize) - 1, 1)}px`)} className="px-3 py-2 border-r">
                    -
                  </button>
                  <input
                    type="text"
        ...
        <div className='flex-1 float-left overflow-y-auto bg-slate-100 hover:opacity-50 transition-transform duration-200 cursor-pointer mt-12'>
          {selectedResume ? (
            <Plain
              ...
              workExperiences={workExperiences.length > 0 ? workExperiences : selectedResume.workExperiences}
              projects={projects.length > 0 ? projects : selectedResume.projects}
              education={education.length > 0 ? education : selectedResume.education}
              languages={languages.length > 0 ? languages : selectedResume.languages}
            ...
            <div className="flex justify-center pt-20">
              <div>
                ...
                <p className='text-xl opacity-30'>Please press the "+" button to create a new one.</p>
              </div>
            </div>
          )}
        </div>
